    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
            <div class="flex flex-row m-5 uppercase">
                ...
                @endif
                @if (Auth::check() && Auth::user()->email === 'marcom@anvaya.com')
                <button wire:click.prevent='openTags' class="{{ $isTags ? 'h-10 px-6 font-semibold rounded-md border text-white bg-slate-900' : 'text-gray-400' }}  py-2 px-4 mr-3 uppercase">
                    Tags
                </button>
                @endif
            ...
            @if($isMenu)
            @include('livewire.menu.cms.beverage.menu.list')
            @endif
            @if ($isCategory)
            @livewire('menu.cms.beverage.category')
            @endif
            @if ($isTags)
            @livewire('menu.beverage-tags')
            @endif
            {{-- @if ($isBoard)
